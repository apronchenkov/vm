...
bool u7_vm_stack_reserve(struct u7_vm_stack* self, size_t capacity) {
  ...
  void* begin = u7_vm_allocator_alloc(self->allocator, capacity);
  if (begin == NULL) {
    return false;
  }
...
void* u7_vm_stack_push(struct u7_vm_stack* self,
  ...
  if (size > self->capacity) {
    ...
    if (!u7_vm_stack_reserve(self, capacity)) {
      return NULL;
    }
  }
  void* const result = u7_vm_memory_add_offset(self->begin, self->size);
